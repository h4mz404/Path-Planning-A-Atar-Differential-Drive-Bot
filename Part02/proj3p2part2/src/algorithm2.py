...
def a_star(start_node, goal_node, display_animation=True):
    ...
    while not open_list.empty():
        ...
        if is_goal(current_node, goal_node):
            ...
            # Extract actions from parent dictionary
            final_actions = []
            node = path[-1]
            while parent[node][0] is not start_node:
                final_actions.append(parent[node][1])
                node = parent[node][0]
            final_actions.append(parent[node][1])
...
set = rospy.ServiceProxy('/gazebo/set_model_state', SetModelState)
state = ModelState()
state.model_name = 'burger'
state.pose.position.x = gazebostart_node[0]
state.pose.position.y = gazebostart_node[1]
state.pose.position.z = 0

yaw = gazebostart_node[2]*math.pi/180
...
# Wait for the publisher to be connected to subscribers
time.sleep(1)

# # Apply actions one by one
for action in act:
    left_rpm, right_rpm = action[0], action[1]
    # Convert RPM to m/s
    left_speed = (left_rpm / 60) * (2 * math.pi * (R/scale) )
    right_speed = (right_rpm / 60) * (2 * math.pi * (R/scale) )

    # Calculate linear and angular velocities
    linear_speed = (left_rpm + right_rpm) * (R/scale) / 2
    angular_speed = (right_speed - left_speed) / (L/scale)
